...
class VideoInput extends Component {
  ...
  handleVideoInput = (event) => {
    ...
    const videoInput = this.refs.VideoInput.value.replace(/(^\w+:|^)\/\//, '//');
            ...
            <Menu
              ...
              onRequestClose={this.handleRequestClose}>
              {
                this.state.videos.map((video, index) => (
                  <MenuItem onClick={this.handleRequestClose.bind(this, video.title, video.src)} key={index}>{video.title}</MenuItem>
                ))
              }
            </Menu>
          </CardContent>
          ...
          <Collapse in={this.state.expanded} transitionDuration="auto" unmountOnExit>
            <CardContent>
              <form id="frmVideo" role="form" onSubmit={this.handleVideoInput}>
                <input
                  type="VideoInput" className="form-control" id="txtVideo" ref="VideoInput" placeholder="Enter a video stream..." name="video"
                />
              </form>
            </CardContent>
          </Collapse>
