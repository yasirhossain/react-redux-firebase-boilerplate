...
export class ChatInput extends Component {
  ...
  onChatSend = event => {
    ...
    if (this.chatFilter(chatMessage) && this.state.chatEnabled) {
      firebaseTools.sendMessage(chatId, chatName, chatMessage, chatReplyName, chatReply, chatAvatar, role, 'chat');
  ...
  chatFilter = message => {
    ...
    if (sanitizedMessage.length < 2) {
      ...
      this.setState({placeholder: 'Sorry, we dont allow saying the same message twice. '});
      return false;
    }
    else return true;
  };
        ...
        <div className={'columns'}>
          ...
          <form id="frmChat" role="form" onSubmit={this.onChatSend} className={`${(this.props.viewable ? '' : 'chat-hide')}`}>
            <input
              ...
              placeholder={ this.props.replyActive ? `Reply to ${this.props.replyMessage.chatName}` : `${this.state.placeholder}`}
              name="chat"
            />
          </form>
